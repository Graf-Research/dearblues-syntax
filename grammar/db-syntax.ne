...
# Enum item
enum_item -> %item_begin %ws:? %variable {% d => d[2] %}
...
table_item_reference -> %item_begin %ws:? %variable %ws fk_access {% d => ({
  type: 'reference',
  column: d[2],
  reference: d[4]
}) %}
table_item_enum -> %item_begin %ws:? %variable %ws %variable {% d => ({
  type: 'enum',
  column: d[2],
  enum: d[4]
}) %}
table_item_primitive -> %item_begin %ws:? %variable %ws %keywords_data_type {% d => ({
  type: 'primitive',
  column: d[2],
  data_type: d[4]
}) %}
...
# Table options
table_options -> table_options %ws:? %comma %ws:? %keywords_options {% d => [...d[0], d[4]] %}
  | %keywords_options

# Table default value
table_dv -> %keywords_default %ws:? %equals %ws:? table_dv_value {% d => d[4] %}
